...
from base.auraahra_company_manager import auraahraCompanyManager
from auraahra import auraahra_middlewares
from auraahra.auraahra_middlewares import _thread_locals
from auraahra.models import auraahraModel, upload_path
from auraahra_audit.models import auraahraAuditInfo, auraahraAuditLog
...
class Department(auraahraModel):
    """
    Department model
    """
    ...
    objects = auraahraCompanyManager()
...
class JobPosition(auraahraModel):
    """
    JobPosition model
    """
    ...
    objects = auraahraCompanyManager("department_id__company_id")
...
class JobRole(auraahraModel):
    ...
    objects = auraahraCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(auraahraModel):
    """
    ...
    objects = auraahraCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(auraahraModel):
    """
...
class EmployeeType(auraahraModel):
    """
    EmployeeType model
    """
...
class EmployeeShift(auraahraModel):
    """
    EmployeeShift model
    """
    ...
    objects = auraahraCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(auraahraModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the auraahra will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = auraahraCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShift(auraahraModel):
    """
    RotatingShift model
    """
...
class RotatingShiftAssign(auraahraModel):
    """
...
class WorkTypeRequest(auraahraModel):
    """
    ...
    def clean(self):
        request = getattr(auraahra_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(auraahraModel):
    """
...
class ShiftRequest(auraahraModel):
    """
    ShiftRequest model
    """
...
class ShiftRequestComment(auraahraModel):
    """
...
class Tags(auraahraModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = auraahraCompanyManager(related_company_field="company_id")
...
class auraahraMailTemplate(auraahraModel):
...
class DynamicEmailConfiguration(auraahraModel):
    """
...
class MultipleApprovalCondition(auraahraModel):
...
class Announcement(auraahraModel):
    """
...
class AnnouncementComment(auraahraModel):
    """
...
class DashboardEmployeeCharts(auraahraModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(auraahraModel):
...
class CompanyLeaves(auraahraModel):
...
class PenaltyAccounts(auraahraModel):
    """
