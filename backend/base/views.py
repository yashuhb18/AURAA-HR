...
from auraahra import auraahra_apps
from auraahra.decorators import (
    delete_permission,
...
from auraahra.group_by import group_by_queryset
from auraahra.auraahra_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from auraahra.methods import get_auraahra_model_class, remove_dynamic_url
from auraahra_audit.forms import HistoryTrackingFieldsForm
from auraahra_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
    ...
    if initialize_database_condition():
        if request.method == "POST":
            ...
            if DB_INIT_PASSWORD == password:
                ...
                return auraahraRedirect(request)
        return render(request, "initialize_database/auraahra_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/auraahra_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/auraahra_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/auraahra_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/auraahra_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/auraahra_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/auraahra_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/auraahra_job_position.html",
    ...
    if request.method == "POST":
        ...
        if employee is None:
            if user.is_superuser:
                # For superusers, we automatically create a dummy employee profile
                # so the system doesn't crash or block them.
                from employee.models import Employee
                employee = Employee.objects.create(
                    employee_user_id=user,
                    employee_first_name=user.first_name or user.username,
                    employee_last_name=user.last_name or "Admin",
                    email=user.email,
                    is_active=True
                )
                print(f"Created automatic employee profile for superuser: {user.username}")
            else:
                messages.error(
                    request,
...
class auraahraPasswordResetView(PasswordResetView):
    """
    auraahra View for Reset Password
    """
    ...
    def form_valid(self, form):
        ...
        if user:
            ...
            if self.request.user.is_authenticated:
                ...
                return auraahraRedirect(self.request)
...
class EmployeePasswordResetView(PasswordResetView):
    """
    auraahra View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not auraahra_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_auraahra_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(auraahra_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/auraahra-logo.png")
    company_name = "auraahra"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from auraahra")
...
@login_required
@hx_request_required
@permission_required("base.change_auraahramailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = auraahraMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_auraahramailtemplate")
...
@login_required
@permission_required("base.delete_auraahramailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = auraahraMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_auraahra_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_auraahra_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_auraahra_model_class(
...
@login_required
@permission_required("auraahra_audit.change_accountblockunblock")
...
@login_required
@permission_required("auraahra_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("auraahra_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("auraahra_audit.change_audittag")
...
@login_required
def get_auraahra_installed_apps(request):
