...
class FilingStatus(auraahraModel):
    """
    FilingStatus model
    """
...
class LoanAccount(auraahraModel):
    """
...
class Reimbursement(auraahraModel):
    """
    Reimbursement Model
    """
            ...
            if self.status == "approved" and self.allowance_id is None:
                if self.type == "reimbursement":
                    ...
                    if bonus_points.points >= self.bonus_to_encash:
                        ...
                        request = getattr(
                            auraahra_middlewares._thread_locals, "request", None
                        )
                        if request:
                            messages.info(
                                request,
...
class ReimbursementrequestComment(auraahraModel):
    """
