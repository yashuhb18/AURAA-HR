"""
auraahra_automations/views/cbvs.py
"""
...
from auraahra.decorators import login_required, permission_required
from auraahra_automations import models
from auraahra_automations.filters import AutomationFilter
from auraahra_automations.forms import AutomationForm
from auraahra_views.generic.cbv import views


@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("auraahra_automations.view_mailautomation"), name="dispatch"
)
class AutomationSectionView(views.auraahraSectionView):
    """
    ...
    template_name = "auraahra_automations/section_view.html"
...
class AutomationNavView(views.auraahraNavView):
    """
    AutomationNavView
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.actions = []
        if self.request.user.has_perm("auraahra_automations.add_mailautomation"):
            self.create_attrs = f"""
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("auraahra_automations.change_mailautomation"), name="dispatch"
)
class AutomationFormView(views.auraahraFormView):
    """
    AutomationFormView
    """
    ...
    template_name = "auraahra_automations/automation_form.html"
...
class AutomationListView(views.auraahraListView):
    """
    AutomationListView
    """
...
class AutomationDetailedView(views.auraahraDetailedView):
    """
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("auraahra_automations.add_mailautomation"), name="dispatch"
)
class LoadAutomationsView(View):
    template_name = "auraahra_automations/load_automation.html"
    ...
    def post(self, request):
        ...
        for template_json in templates_raw:
            if template_json["pk"] in required_template_pks:
                ...
                existing = auraahraMailTemplate.objects.filter(
        ...
        for automation_json in selected_automations:
            ...
            mail_template = auraahraMailTemplate.objects.filter(
                body=template_body
            ).first()
