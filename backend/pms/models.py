...
from auraahra.models import auraahraModel
...
class KeyResult(auraahraModel):
    ...
    history = auraahraAuditLog(bases=[auraahraAuditInfo])
...
class EmployeeObjective(auraahraModel):
    ...
    history = auraahraAuditLog(bases=[auraahraAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
...
class Comment(models.Model):
    ...
    history = auraahraAuditLog(excluded_fields=["comment"], bases=[auraahraAuditInfo])
    objects = auraahraCompanyManager(
...
class QuestionTemplate(auraahraModel):
...
class Question(auraahraModel):
    ...
    objects = auraahraCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(auraahraModel):
    ...
    objects = auraahraCompanyManager("question_id__template_id__company_id")


class Feedback(auraahraModel):
...
class EmployeeBonusPoint(auraahraModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
