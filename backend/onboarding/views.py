...
from base.models import auraahraMailTemplate, JobPosition
...
from auraahra.group_by import group_by_queryset as general_group_by
from auraahra.http.response import auraahraRedirect
from auraahra_documents.models import Document
...
def candidates_view(request):
    ...
    mail_templates = auraahraMailTemplate.objects.all()
...
def email_send(request):
    ...
    bodys = list(
        auraahraMailTemplate.objects.filter(id__in=template_attachment_ids).values_list(
            "body", flat=True
        )
    )
