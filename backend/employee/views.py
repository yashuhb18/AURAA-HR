...
from auraahra.filters import auraahraPaginator
from auraahra.group_by import group_by_queryset
from auraahra.auraahra_settings import auraahra_DATE_FORMATS
from auraahra.http import auraahraRedirect
from auraahra.methods import get_auraahra_model_class
from auraahra_audit.models import AccountBlockUnblock, HistoryTrackingFields
from auraahra_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
)
from auraahra_documents.models import Document, DocumentRequest
...
@login_required
@manager_can_enter("auraahra_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="auraahra_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("auraahra_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "auraahra_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "auraahra_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("auraahra_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("auraahra_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("auraahra_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("auraahra_documents.delete_document"):
...
def file_upload(request, id):
    ...
    if not can_access_document(
        request, document_item, "auraahra_documents.change_document"
    ):
        return auraahraRedirect(
...
def view_file(request, id):
    ...
    if not can_access_document(
        request, document_obj, "auraahra_documents.view_document"
    ):
        return auraahraRedirect(
...
@login_required
@hx_request_required
@manager_can_enter("auraahra_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = auraahraPaginator(qryset, get_pagination())
...
def employee_view_update(request, obj_id, **kwargs):
    ...
    return auraahraRedirect(request, fallback_url="/employee/employee-view")
...
def employee_delete(request, obj_id):
    ...
    try:
        ...
        return redirect(employee_view)
    return auraahraRedirect(request, fallback_url=f"/view={view}")
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_auraahra_model_class(
                    ...
                    Stage = get_auraahra_model_class(
                    ...
                    OnboardingStage = get_auraahra_model_class(
                    ...
                    OnboardingTask = get_auraahra_model_class(
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        auraahra_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_auraahra_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_auraahra_model_class(
