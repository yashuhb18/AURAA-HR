...
class CompanyLeave(auraahraModel):
...
class AvailableLeave(auraahraModel):
...
class LeaveRequest(auraahraModel):
    employee_id = models.ForeignKey(
...
class LeaverequestComment(auraahraModel):
    """
...
class LeaveAllocationRequest(auraahraModel):
    leave_type_id = models.ForeignKey(
        LeaveType, on_delete=models.PROTECT, verbose_name=_("Leave type")
    )
    employee_id = models.ForeignKey(
...
class LeaveallocationrequestComment(auraahraModel):
    """
...
class RestrictLeave(auraahraModel):
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaveRequest(auraahraModel):
        leave_type_id = models.ForeignKey(
...
class LeaveGeneralSetting(auraahraModel):
    """
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaverequestComment(auraahraModel):
        """
...
class EmployeePastLeaveRestrict(auraahraModel):
...
if apps.is_installed("attendance"):
    ...
    class OverrideLeaveRequests(LeaveRequest):
        ...
        #     """
        #     WorkRecords = get_auraahra_model_class(
